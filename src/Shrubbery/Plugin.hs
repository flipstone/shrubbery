...
import GHC.Plugins (Plugin(..), defaultPlugin, CoreM, ModGuts(..), CoreToDo(..), TyCon, DataCon, mkSysLocalM, fsLit, getHscEnv, tyConDataCons, Type, mkTyConApp, Alt(..), AltCon(..), Boxity(..), mkStrLitTy, PkgQual(..), ModuleName, mkFastString, mkModuleNameFS, manyDataConTy, Id, AnnTarget(NamedTarget), deserializeWithData, findAnns, mkAnnEnv, tyConName, errorMsg, text, Name, mkTcOcc)
...
lookupCaseTyCon = do
  hscEnv <- getHscEnv
  result <- liftIO (findImportedModule hscEnv typeLevelModule NoPkgQual)
  case result of
    Found _loc mod_ -> do
      caseName <- lookupOrig mod_ (mkTcOcc "Case")

      lookupTyCon caseName
    _ -> do
