...
import GHC.Generics (Generic(from, to), Rep, M1(M1,unM1), K1(K1,unK1), (:+:)(L1, R1), U1(U1))
import Unsafe.Coerce (unsafeCoerce)

import Shrubbery.BranchIndex (BranchIndex, firstIndexOfType, appendTypesToIndex, prependTypesToIndex, splitIndex)
...
-}
genericDissect :: (Generic a, GenericUnion (Rep a))
...
{- |
  ...
  might not actually want to do this.
-}
genericUnifyWithIndex :: (Generic a, GenericUnion (Rep a))
...
{- |
  ...
  might not actually want to do this.
-}
genericFromUnion :: (Generic a, GenericUnion (Rep a))
