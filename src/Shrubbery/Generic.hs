...
one line implementations. For example:
...
Warning: 'GenericBranchTypes' orders the list of types according to the order of the constructors in
the definition of your sum type (@MySum@ above). The results in any users of 'dissect' or
'unifyWithIndex' to be implicitly dependent on the ordering. In the best case scenario, this means
re-ordering constructors might produce a compilation error for a user of the type. In the worst
case, it might slightly change the callers logic if the types happen to match. In the example above
flipping the order of @MySumIntA@ and @MySumIntB@ will not produce any type error detectable by the
compiler, but will nonetheless result in the use of constructors being flipped in all usages of
'dissect' and 'unifyWithIndex'. This is almost certainly not what users of those functions were
expected. It is strongly recommended that you create a specific and unique type to represent branch
of your sum type if you plan to use 'genericDissect' and 'genericUnifyWithIndex' with it.
